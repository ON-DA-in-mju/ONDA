...
data class OperationCancelUiState(
    val info: OperationCancelInfo = OperationNoticeMapper.cancelInfo(""),
...
class OperationCancelViewModel(
    savedStateHandle: SavedStateHandle,
) : ViewModel() {

    private val _uiState = MutableStateFlow(
        OperationCancelUiState(
            info = OperationNoticeMapper.cancelInfo(
    ...
    fun onConfirm() {
        viewModelScope.launch { _events.emit(OperationCancelEvent.ConfirmAndGoHome) }
    }

    fun onBack() {
        viewModelScope.launch { _events.emit(OperationCancelEvent.NavigateBack) }
    }

    fun onAlarmClick() {
        viewModelScope.launch { _events.emit(OperationCancelEvent.OpenAlarms) }
    }

    fun onHistoryClick() {
        viewModelScope.launch { _events.emit(OperationCancelEvent.OpenHistory) }
    }

    fun onSettingsClick() {
        viewModelScope.launch { _events.emit(OperationCancelEvent.OpenSettings) }
    }
