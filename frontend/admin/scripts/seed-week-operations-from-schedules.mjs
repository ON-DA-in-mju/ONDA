...
async function main() {
  ...
  // route_stops 첫/끝 → origin/destination_stop_id (3NF)
  ...
  for (const day of days) {
    ...
    for (const sch of daySchedules) {
      ...
      rows.push({
        ...
        round: 1,
        origin_stop_id: endsByRoute.get(sch.route_id)?.first ?? null,
        destination_stop_id: endsByRoute.get(sch.route_id)?.last ?? null,
