...
const GIHEUNG = ['08:15', '08:25', '09:05', '09:10', '10:00', '10:05', '12:00', '13:00', '14:00', '15:15', '16:15', '17:15', '18:15', '19:15']
const MJU_SEM = [
...
async function main() {
  ...
  const names = ['기흥역 통학버스', '명지대역 셔틀', '시내 셔틀']
  const idByName = {}
  ...
  let rows = []
  rows = rows.concat(expand(idByName['기흥역 통학버스'], GIHEUNG, WEEKDAYS, 'SEMESTER'))
  rows = rows.concat(expand(idByName['명지대역 셔틀'], MJU_SEM, WEEKDAYS, 'SEMESTER'))
  rows = rows.concat(expand(idByName['시내 셔틀'], CITY_SEM, WEEKDAYS, 'SEMESTER'))
  rows = rows.concat(expand(idByName['시내 셔틀'], CITY_WE, WEEKEND, 'SEMESTER'))
  rows = rows.concat(expand(idByName['명지대역 셔틀'], MJU_VAC, WEEKDAYS, 'VACATION'))
  rows = rows.concat(expand(idByName['시내 셔틀'], CITY_VAC_WD, WEEKDAYS, 'VACATION'))
  rows = rows.concat(expand(idByName['시내 셔틀'], CITY_WE, WEEKEND, 'VACATION'))
  ...
  const check = await fetch(
    `${url}/rest/v1/schedules?select=id&route_id=in.(${ids.join(',')})`,
    { headers: { apikey: key, Authorization: `Bearer ${lb.access_token}`, Prefer: 'count=exact' }, method: 'HEAD' },
  )
